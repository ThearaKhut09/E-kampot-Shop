...
<div {{ $attributes->merge(['class' => 'relative']) }} x-data="{ open: false }">
    ...
    <button
        ...
        class="{{ $triggerSizeClass }} relative inline-flex items-center justify-center overflow-hidden rounded-full ring-2 ring-transparent transition-all duration-200 hover:ring-primary-300 focus:outline-none focus:ring-primary-500 cursor-pointer"
    >
        <img
    ...
    >
        @foreach ($locales as $localeCode => $locale)
            <button
                type="button"
                @click="switchLanguage('{{ $localeCode }}'); open = false"
                class="flex w-full items-center gap-3 rounded-xl px-2.5 py-2 text-left text-sm transition-all duration-200 cursor-pointer hover:scale-105 {{ $currentLocale === $localeCode ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300' : 'text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/50' }}"
...
function switchLanguage(locale) {
    ...
    fetch('{{ route("language.switch", ":locale") }}'.replace(':locale', locale), {
        method: 'POST',
        headers: {
